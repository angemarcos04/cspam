...
Then check the protected readiness endpoint. It should report `checks.dashboard.columns.students.status: ok`, `checks.dashboard.columns.schools.status: ok`, `checks.dashboard.columns.performanceMetrics.status: ok`, `checks.dashboard.columns.indicatorSubmissionItems.status: ok`, and `checks.dashboard.data.students.status: ok`.
...
```bash
php artisan queue:work --verbose --queue=mail,broadcasts,default --tries=3 --timeout=90
```
...
Copy all shared web service environment variables to the worker, especially `APP_KEY`, all `DB_*` values, all `MAIL_*` values, `LOG_CHANNEL=stderr`, and `LOG_LEVEL=info`. Configure the worker itself with `QUEUE_CONNECTION=database`, `CSPAMS_QUEUE_NAMES=mail,broadcasts,default`, and `CSPAMS_MONITOR_MFA_QUEUE=mail`. Prefer a Render Environment Group for shared values while keeping worker-only queue settings on the worker service.

The worker script does not run migrations or seeders. It prepares Laravel cache and runs:

```bash
php artisan queue:work --verbose --queue=mail,broadcasts,default --tries=3 --timeout=90 --sleep=3
```
