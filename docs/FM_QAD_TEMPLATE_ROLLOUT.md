...
The initial dry run must report empty `Inactive existing`, `Label conflicts`, `Missing catalog`, `Missing files`, and `Invalid files` results. On an empty version library it should report ten would-import decisions. The real import should then import those revisions (or skip hashes already present), and the post-import dry run should report zero would-import and ten would-skip decisions. Any inactive-existing, label-conflict, missing-catalog, missing-file, invalid-file, or audit issue makes the command fail and must stop the rollout.
...
`Label conflicts` reports a normalized revision label that already exists with different DOCX content. This is always a failed, non-destructive preflight. `--force` does not replace its blob, reactivate it, or create a duplicate; review the existing revision and retained asset manually.
...
All ten configured permanent form identities are always enabled. Template Management controls revisions, not permanent-form availability. The deployment seeder restores a disabled configured row, while the read-only integrity audit reports `disabledConfiguredForms` and user-facing direct routes return 404 until the drift is corrected. The audit reports an enabled non-configured row under `unexpectedEnabledForms`, but does not treat it as part of permanent-catalog version or Academic Year coverage. Disabled historical rows are preserved and ignored by permanent-catalog health checks.

New legacy versions, blobs, conflict archival, activation, upload audit rows, and activation audit rows are committed in one importer-only database transaction. If storage, activation, or either audit insert fails, every import change rolls back together. Realtime success events are dispatched only after that transaction returns successfully. Normal Monitor uploads continue to save legitimate drafts.
