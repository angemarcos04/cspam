...
class LegacyFmQadTemplateImporter
{
    ...
    public function run(bool $dryRun = false, ?string $scopeId = null, bool $force = false): array
    {
        ...
        $result = $dryRun
            ? ['checked' => 0, 'wouldImport' => 0, 'wouldSkip' => 0, 'wouldReactivate' => 0, 'missing' => [], 'invalid' => []]
        ...
        foreach (config('fm_qad.forms', []) as $definition) {
            ...
            if (! $form) {
                $result['missing'][] = $definition['scope_id'].':catalog';
            ...
            if ($existing) {
                $reactivate = $force && $existing->status !== FmQadTemplateVersion::ACTIVE;
                if ($dryRun) {
                    $result[$reactivate ? 'wouldReactivate' : 'wouldSkip']++;
                } elseif ($reactivate) {
