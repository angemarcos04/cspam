...
Artisan::command('cspams:import-fm-qad-templates {--dry-run} {--force} {--form=}', function (LegacyFmQadTemplateImporter $importer): int {
    ...
    $result = $importer->run($dryRun, $this->option('form') ?: null, (bool) $this->option('force'));
    $headers = $dryRun
        ? ['Checked', 'Would Import', 'Would Skip', 'Would Reactivate', 'Missing', 'Invalid']
        : ['Checked', 'Imported', 'Skipped', 'Reactivated', 'Missing', 'Invalid'];
    $row = $dryRun
        ? [$result['checked'], $result['wouldImport'], $result['wouldSkip'], $result['wouldReactivate'], implode(', ', $result['missing']), implode(', ', array_keys($result['invalid']))]
        : [$result['checked'], $result['imported'], $result['skipped'], $result['reactivated'], implode(', ', $result['missing']), implode(', ', array_keys($result['invalid']))];
