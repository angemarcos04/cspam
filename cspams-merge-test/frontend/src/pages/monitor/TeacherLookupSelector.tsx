...
interface TeacherLookupSelectorProps {
  dropdownId: string;
  isOpen: boolean;
  ...
  isSyncing: boolean;
  query: string;
  filteredOptions: TeacherLookupOption[];
  allOptions: TeacherLookupOption[];
  selectedTeacherId: string | null;
  onToggle: () => void;
  onQueryChange: (value: string) => void;
  onClearQuery: () => void;
  onClearSelection: () => void;
  onSelectOption: (option: TeacherLookupOption) => void;
}

export function TeacherLookupSelector({
  dropdownId,
  isOpen,
          ...
          <div className="relative max-h-72 overflow-y-auto overscroll-contain p-1 pr-1 [scrollbar-gutter:stable]">
            <button
              ...
              className={`block w-full px-2.5 py-1.5 text-left text-xs transition ${
                !selectedTeacherId ? "bg-primary-50 text-primary-800" : "text-slate-700 hover:bg-slate-50"
              }`}
            >
              All teachers
            </button>
            {filteredOptions.map((option) => (
              <button
                ...
                className={`block w-full px-2.5 py-1.5 text-left text-xs transition ${
                  selectedTeacherId === option.id ? "bg-primary-50 text-primary-800" : "text-slate-700 hover:bg-slate-50"
                }`}
              >
